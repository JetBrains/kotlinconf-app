...
fun UserScreen(data: AggregatedData, userId: String, onBack: () -> Unit) {
    ...
    Div(attrs = { classes("crumbs") }) {
        Button(attrs = { classes("link"); onClick { onBack() } }) { Text("← Back to overview") }
    }

    Div(attrs = { classes("card") }) {
        Div(attrs = { classes("user-meta") }) {
            H1 { Text(pseudoName(userId)) }
            Span(attrs = { classes("pill") }) { Text("${votes.size} vote" + if (votes.size == 1) "" else "s") }
            Span(attrs = { classes("pill") }) { Text("${feedback.size} comment" + if (feedback.size == 1) "" else "s") }
        }
...
private fun scoreLabel(score: Score?): String = if (score != null) "${score.emoji()} $score" else "—"
